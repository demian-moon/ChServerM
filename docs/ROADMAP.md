...
- [ ] ⚠ 피어 링크 배선 — **계층 선택이 먼저다**(ADR-0049). `ChServerMClient` 는 Hosting 에 있고 의존 방향은 `Hosting → 어댑터 → Core` 이므로 `ChServerM.Cluster` 가 참조할 수 없다. 방안 셋: Hosting 안에 두기 / `ChServerM.Cluster.Hosting` 신설 / Core 에 `IPeerLink` 축(구현이 하나뿐인 추상화는 껍데기다). **재접속 정책도 함께 정한다** — `ClientBuilder` 는 "재접속을 프레임워크가 감추면 상위 계층이 세션 재수립을 건너뛴다"고 못박아 뒀는데, 상태 없는 피어 링크에도 같은 논리가 적용되는지
