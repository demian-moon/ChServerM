...
- [ ] 커넥션 풀 관리 — StackExchange.Redis 멀티플렉서 공유 규약은 어댑터 문서에 있으나(소유권=호출자), 풀 크기·재연결 정책 튜닝은 미착수
