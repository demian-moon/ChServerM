...
- [ ] `ChServerM.SourceGen` — 메시지 ID → 핸들러 스위치 테이블 생성 (진행 중: 2026-08-05 1차 증분(ADR-0014) — `[MessageHandler]` 발견 + `MapGeneratedHandlers` 등록 생성 + 통합 테스트 종단 검증. **스위치 문 직생성은 남았다** — 현 배열 인덱싱 라우팅과의 성능 차이를 벤치마크로 확인한 뒤 판단("측정 없는 최적화 금지"))
