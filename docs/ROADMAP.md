...
- [ ] ⚠ 파티셔닝 / 라우팅 전략 — 상태 유지 노드에 어떤 키로 라우팅하는가. **`PartitionKey.ToIndex(노드 수)` 를 그대로 쓰면 안 된다** — 노드가 하나 바뀔 때 **거의 모든 키가 재배치된다**(프로세스 내 파티션은 개수가 고정이라 괜찮았지만 클러스터는 아니다). 일관 해싱 또는 랑데뷰(HRW)가 필요하며, 노드 이름을 링에 올리는 해시는 **Core 밖**에 둔다(Core 는 서드파티 의존 0이라 좋은 해시를 들일 수 없다, ADR-0047)
- [ ] 노드 간 통신 — 내부 전송 경로
