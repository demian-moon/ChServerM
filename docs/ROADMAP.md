...
- [ ] ⚠ **프로토콜 버전 협상** — 현재 유일한 정책이 "버전 불일치 → 커넥션 종료"인데, 무중단 롤링 배포(Phase 15) 중에는 서버·클라 버전 불일치가 **정상 상태**라 구조적으로 충돌한다(2026-08-04 감사). 핸드셰이크에서 상호 지원 버전을 교섭하는 경로가 필요하다 — Phase 9 핸드셰이크 설계에 포함시킨다 (진행 중: **설계는 확정** — THREAT-MODEL 5절 요구사항 R-1~R-5 + ADR-0017 결정 3, TLS 채널 안에서 ClientHello`[min,max]`→ServerHello 1왕복·헤더 v1 영구 동결·직렬화 축 비의존. 와이어 레이아웃 구현·테스트가 남았다)
...
(2026-08-05 **두 조건 모두 충족** — 매핑은 `2842d2d`, 인증 전 차단 테스트는 `1b3fc20`. 미완료 항목은 게이트 조건과 무관하게 계속 진행)
