...
- [ ] 리소스 상한 — 최대 커넥션 수, 커넥션당 메모리 상한, 전체 메모리 워터마크 (진행 중: 최대 커넥션 수(정적 `MaxConnections`)·커넥션당 메모리(Phase 5 실측 ~8KB)는 있음. 전체 메모리 워터마크는 **신호가 생겼다** — 2026-08-07 `b88267b` 의 `MemoryLoadLevelSource`(비율 기반, 컨테이너 제한 자동 추종)가 열화의 입력으로 쓰인다. **남은 것은 그 신호를 `IAdmissionControl` 에 잇는 어댑터**(메모리 높을 때 신규 연결 거부) — 작은 배선이다)
