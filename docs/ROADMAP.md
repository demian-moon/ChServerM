...
- [ ] `ChServerM.Transport.Tcp` — accept 루프 + 수신/송신 펌프. `TcpClient`/`NetworkStream`을 쓰지 않는다. 수락 루프가 일시적 오류(개별 연결 끊김)와 치명적 오류(수락 소켓 사망)를 구분한다 — 구분하지 않으면 조용히 수용을 멈추거나 CPU를 태운다 (진행 중: 구현·테스트 완료. **DoD 조건 3 미충족 — 에코 RPS·레이턴시 벤치마크 수치가 없다.** 2026-08-04 감사에서 `[x]` 표기가 완료 부풀리기로 판정돼 되돌림)
- [ ] 백프레셔 — pause/resume 임계값을 옵션으로 노출. **최대 프레임보다 커야 한다는 제약을 조립 시점에 검사한다**(ADR-0007). `WaitForDataBeforeAllocating`(0바이트 수신)으로 유휴 커넥션이 버퍼를 붙들지 않게 — 1만 유휴 접속 × 4KB = 40MB 절감. 레거시는 커넥션당 64KB 상수(= 640MB) (진행 중: 구현 완료. **DoD 조건 3 미충족 — "40MB 절감"은 계산값이고 부하 실측이 없다.** 2026-08-04 정정)
...
- [ ] **ADR-0008 프레임당 할당 교정 (측정 기반)** — 메시지 단위 측정에서 프레임당 ~170-184 B 실측(ENV-B). 원인 둘: (1) 배타 완료 신호의 `RunContinuationsAsynchronously` 가 스레드풀 큐 항목을 프레임마다 할당, (2) 소비 루프 `WaitForWork` 의 `WaitToReadAsync().AsTask()` 가 대기마다 `Task` 할당 — (2)는 구 설계에도 있었으나 청크 벤치마크가 가려왔다. 교정 후보: `IThreadPoolWorkItem` 직접 구현 / 재사용 동기 대기 프리미티브. before/after 는 `PartitionExclusiveBenchmarks` 로
- [ ] **소비 스레드 상시 바쁨 구성의 배타 왕복 측정** — 현 측정(파티션당 생산자 1, 프레임마다 잠들고 깸)은 최악 사례라 P=8부터 역확장이 나온다. 게시자 다수가 소비 스레드를 계속 바쁘게 만드는 구성이 실제 부하에 가깝다 (`BENCHMARKS.md` 미측정 항목)
...
- [ ] ⚠ **프로토콜 버전 협상** — 현재 유일한 정책이 "버전 불일치 → 커넥션 종료"인데, 무중단 롤링 배포(Phase 15) 중에는 서버·클라 버전 불일치가 **정상 상태**라 구조적으로 충돌한다(2026-08-04 감사). 핸드셰이크에서 상호 지원 버전을 교섭하는 경로가 필요하다 — Phase 9 핸드셰이크 설계에 포함시킨다
