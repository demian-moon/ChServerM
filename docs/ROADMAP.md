...
- [ ] ⚠ **기본값 결정 — 4-인자 생성자가 `ArrayPool<byte>.Shared` 를 쓰는 것이 함정이다** (ADR-0051 열린 결정). 깊은 큐를 잡은 사용자가 경고도 오류도 없이 33% 손해를 본다. **선결 조건: 전용 풀의 실제 보유 메모리 상한 측정** — 기본 `MaxPayloadLength` 가 1 MiB 라 버킷 합산이 GB 단위가 될 수 있어, 그 값을 모르면 "옵션에서 풀을 직접 만든다" 안을 고를 수 없다
- [ ] ⚠ `PooledBufferWriter` 를 **1만 커넥션 규모**로 재측정 — ADR-0016 의 "정상 상태 할당 0" 은 얕은 조건의 값이다. 프레임워크 내부 사용처는 현재 0(공개 빌딩 블록)이지만 문서가 그 사용법을 권장한다
