...
`LegacyServer/`와 `LagacyClient/`는 **저장소에 포함하되 승계 대상이 아니다**.

> **2026-08-03 변경.** 이전에는 커밋하지 않고 로컬 참조 전용으로 뒀다(`.gitignore` 로 차단).
> 다른 작업 환경에서 같은 조건으로 이어가려면 참조 대상이 저장소에 있어야 하므로 포함으로 바꿨다.
> **승계 방침은 그대로다** — 솔루션(`ChServerM.slnx`)에 넣지 않으므로 빌드·테스트·CI 가
> 레거시를 컴파일하지 않고, 코드는 새로 작성하거나 개선해서 옮긴다.
