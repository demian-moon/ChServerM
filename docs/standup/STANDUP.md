...
**최종 갱신**: 2026-08-08
**현재 단계**: **Part III 마무리 → Phase 12 착수**. Phase 9 보안 ✅ / **Phase 11 관측 ✅ 완결(9/9)** / Phase 10 복원력 7/10(게이트 실질 충족) / Phase 12 성능 검증 2/10(할당 게이트·기준선 확정)
**진행률**: 113/222 항목 (Phase 0 `13/17` · 1 `15/21` · 2 `7/11` · 3 `4/6` · 4 `10/10` · 5 `12/12` · 6 `7/7` · 7 `5/7` · 8 `9/16` · 9 `13/13` · 10 `7/10` · **11 `9/9` ✅** · 12 `2/10`)
> 집계는 **하위 항목 포함** 기준이다(최상위만 세면 109/218). 2026-08-08 프로그램 재집계로 기준을 명시했다.
...
- **Part II 재검증 ✅ (2026-08-07 `3b8c238`)** — ADR-0005 를 **근사가 아닌 실측**으로 방어했다.
  프로세스 어피니티로 물리 코어를 제한해(1·2·4·8·16) 재측정: **16코어 14.67×(91.7%)**,
  같은 머신의 순수 ALU 천장(14.81×)의 **99.0%** — 남은 손실은 실행 모델이 아니라 하드웨어·OS 몫.
  **선행 발견**: SMT 형제가 인접 쌍이라 기존 안내(`/affinity F`=4코어)가 틀렸다(실제 2코어)
- **Phase 10 리소스 상한 ✅ / Phase 11 로그 레벨 ✅ (2026-08-08 `b3b7fa4`)** —
  `LoadLevelAdmissionControl` 이 부하 신호를 수용 축에 잇는다(**열화가 먼저 `Elevated`,
  수용 거부가 나중 `Critical`** — 뒤집으면 재시도가 accept 부하를 더한다). 로그 레벨 런타임
  변경은 **MEL 어댑터로 이미 성립**함을 실제 `LoggerFactory` 로 검증(프레임워크 코드 0)
- **Phase 11 관측 ✅ 완결 9/9 (2026-08-08 `13b9694`)** — 마지막 항목 **런타임 진단**:
  Core `IDiagnosticsSource` 옵트인 + `DiagnosticsService`(구역별 try/catch) + 전송·실행 모델
  자동 등록 + 버퍼 풀 소스. **메트릭의 여집합**(요청 시점 고카디널리티 스냅샷)이라는 것이
  설계 근거다. **커넥션은 집계+상한 표본 20**(전체 덤프는 MB 급 응답 + 무인증 admin 에
  클라이언트 주소 노출), HTTP 는 **옵트인**(기본 미노출)
- **Phase 12 착수 ✅ (2026-08-08 `c406106`)** — **할당 회귀 게이트**: 코덱에만 있던 할당 고정을
  **조립된 파이프라인**으로 확장(추적·열화 fast-path, 열화 거부 경로, 속도 제한, 합성 형태
  전부 프레임당 0). **★ 게이트가 고의 회귀를 실제로 잡는지까지 고정** — Phase 12 게이트 조건
  그 자체. `BENCHMARKS.md` 가설 표를 **실측 기준선**으로 대체 + 회귀 방어 상태 표
- 테스트 **748개** 통과, 전체 게이트(-WarnAsError 클린 빌드·audit·AOT publish+실행) 통과

## 진행 중

- **Phase 12 잔여(8개)** — LoadRunner 램프업 무한 루프 수정(알려진 버그·고아 프로세스) /
  CI 벤치마크 회귀 게이트(노이즈 전략·CI 환경 결정 필요) / 확장성 회귀 게이트(어피니티 방법은
  2026-08-07 에 확립) / NBomber 종단 부하 / 프로파일링 문서 / GC 튜닝 / AOT vs JIT / 경쟁 비교
- **Phase 10 잔여** — 정식 24h soak 를 CI 에 스케줄(하네스 완비, 실행만 남음)
...
1. **LoadRunner 램프업 무한 루프 수정** — 알려진 버그(2026-08-05 발견). 대상 서버가 없거나
   죽으면 연결 실패만 세며 영원히 돌아 **고아 프로세스**를 만든다. 작고 확실한 항목
2. **CI 벤치마크·확장성 회귀 게이트** — 노이즈 전략(반복·중위값)과 CI 환경 결정이 선행.
   확장성은 어피니티 마스크 방법이 이미 확립돼 있다
3. **Part IV 진입 검토(Phase 13 세션 & 영속화)** — 여기서 **첫 아웃바운드 의존**이 생기므로
   보류해 둔 서킷 브레이커(ADR-0027)가 실물과 함께 온다
4. **Phase 10 마무리** — 정식 24h soak 를 CI 에 스케줄
...
- **게이트는 스스로 증명해야 한다** — "회귀를 잡는다" 는 주장에도 테스트가 필요하다. 고의 회귀를
  넣어 게이트가 **실패하는 것까지** 고정해야 게이트가 게이트다. 통과만 보는 테스트는 언제
  무력해졌는지 알 수 없다(Phase 12 게이트 조건이 정확히 이것을 요구한다)
- **측정 도구의 범위가 테스트의 신뢰성을 정한다** — `GC.GetTotalMemory` 는 프로세스 전역,
  `GetAllocatedBytesForCurrentThread` 는 스레드 지역이다. 병렬 테스트에서 전자는 남의 할당을
  세어 플래키가 된다(실제로 그렇게 깨졌다). **시간 기반 게이트보다 할당 게이트를 먼저 세운
  이유**도 같다 — 할당은 결정적이다
- **기본값은 덜 노출하는 쪽으로** — 진단 HTTP 경로를 옵트인(기본 미노출)으로 뒀다. 그 안에는
  클라이언트 주소가 들어간다. **켜는 실수보다 안 켜는 실수가 덜 나쁘다**
- **측정 전에 측정 도구를 측정한다** — 어피니티 마스크가 물리 코어를 고르는지 확인하지 않았다면
  확장성 숫자가 통째로 2배 틀렸고, 그 숫자로 ADR 을 "검증했다" 고 적었을 것이다. 게다가 **첫
  프로브도 틀린 답을 줬다**(의존 체인 워크로드는 SMT 도 1.96× 를 낸다 — 고-ILP 라야 판정이 선다).
  **무엇을 재는지는 어떻게 재는지에 달려 있다**
- **대조군을 설계에 넣는다** — "1코어에서는 파티션을 늘려도 평탄해야 한다" 는 예측이 어피니티가
  실제로 걸렸음을 증명한다. 측정값만 있고 대조군이 없으면 "제한이 걸렸는가" 를 답할 수 없다
- **단계적 대응은 순서가 설계다** — 부하가 오를 때 열화(비필수 폐기)가 먼저, 수용 거부가 나중이다.
  뒤집으면 조금만 밀려도 신규 사용자가 막히고 **그 재시도가 부하를 더한다**. 두 방어가 **같은
  신호를 공유**해야 운영자가 상태를 하나로 설명할 수 있다
...
- 상세 이력: `docs/standup/history/` (최근: `2026-08-08.md`)
