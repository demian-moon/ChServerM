...
namespace EcsClientLibM
{
	...
	//public class MemPkFactoryForClient : AbMemPkFactory
	//{
	//	ClientM _clientM;

	//	public MemPkFactoryForClient(ClientM clientM, TcpClient tc, CancellationTokenSource cts) : base(tc, cts)
	//	{
	//		_clientM = clientM;
	//	}
	...
	//	{
	//		await _clientM.SendMemPk(memPk, Cts).ConfigureAwait(false);
	//	}
	...
	//	{
	//		await _clientM.SendEncMemPk(encMemPk, Cts).ConfigureAwait(false);
	//	}
	//}

	/// <summary>
	/// IoPipe Write에서 쓰이는 클라 쪽 disconnect Process 클래스
	/// </summary>
	public class ClientDisconnectProcess : AbDisconnectProcess
	{
		ClientM _clientM;

		public ClientDisconnectProcess(ClientM clientM)
		{
			_clientM = clientM;
		}

		/// <summary>
		/// 클라이언트 Disconnect 프로세스
		/// </summary>
		/// <param name="tc"></param>
		override public async ValueTask DisconnectProcess(TcpClient tc)
		{
			Debug.WriteLine("나클라임: Fin 받음 -------------- 접속 최종 종료야 킹받네");

			UserM user = ClientM.GetUser(tc);
			if (user.IsExist)
			{
				user.DisconnectProcess();    // 클라 유저가 가진 자원들 모두 지우기
				await _clientM.AppFinish(user).ConfigureAwait(false);

				ClientM.RemoveUser(tc);    // 클라 _dicUsers에서  유저를 지움
				Debug.WriteLine($"### 로그인 Ok 후에 클라 종료되서 바로 처리 {user.Oid} ###");
			}
			else
			{
				CompressAndEncryptManM.TryRemove(tc, out CompressAndEncryptM _); // 혹시 살아 있을 수 있는 글로벌 암호화 객체 삭제 (유저 로그인 전에 Tc close되면)

				ClientM.gDisconnectTimer.AddOrUpdateTimer(tc, new TimerM_ClaUser_Delay_Disconnect(_clientM, tc), TimeSpan.FromSeconds(1), Timeout.InfiniteTimeSpan);
			}
		}
	}


	public class IoPipelineClaM
	{
		// 클라이언트용 파이프라인 (MemPk 만든후 저장하는 방식이 서버와 다름)
		static public async Task PipelineForClientAsync(TcpClient tc, ClientM clientM)
		{
			CancellationTokenSource cts = new CancellationTokenSource();
			PipeOptions pipeOption = new PipeOptions(null, null, null, -1, -1, -1, false); // 스레드 풀에서 돌린다. 

			Pipe pipe = new Pipe(pipeOption);
			Task pipeWritingTask = ClaFillPipeAsync(tc, pipe.Writer, cts, new ClientDisconnectProcess(clientM));
			Task pipeReadingTask = ClaReadPipeAsync(tc, pipe.Reader, cts, clientM);
		...
		static public async Task ClaFillPipeAsync(TcpClient tc, PipeWriter pipeWriter, CancellationTokenSource cts, AbDisconnectProcess disCon)
		{
			...
			while (true)
			{
				try
				{
					...
					try
					{
						//int nReadByte = await netStream.ReadAsync(memory, ct).ConfigureAwait(false);  //  추후 .Net으로 프레임워크를 바꾸면 수정 할 것

						byte[] buf = new byte[memory.Length];
						int nReadByte = await netStream.ReadAsync(buf, 0, buf.Length, ct).ConfigureAwait(false);
						new Span<byte>(buf).CopyTo(memory.Span);
			...
			finally
			{
				...
				{
					Debug.WriteLine($"클라 넷스트림 정리 작업 중 오류: {cleanupEx.Message}");
				}
			}
		...
		static public async Task ClaReadPipeAsync(TcpClient tc, PipeReader pipeReader, CancellationTokenSource cts, ClientM clientM) // 읽은 헤더 사이즈
		{
			...
			try
			{
				...
				while (true)
				{
					...
					try
					{
						...
						while (true)
						{
							...
							else if (_toReadState == eToReadState.ENC_PK_DATA)
							{
								...
								// 압축 암호화된 MemPk를 전송
								await clientM.SendEncMemPk(encMemPk, cts).ConfigureAwait(false);  // 여기 이후 shared 메모리 return!!
							...
							else if (_toReadState == eToReadState.CONTENT_HEAD)
							{
								...
								if (_toReadDataLen == 0) // 컨텐츠 데이터가 null이면 
								{
									...
									await clientM.SendMemPk(memPk, cts).ConfigureAwait(false);
					...
					catch (OperationCanceledException oce)
					{
						...
						{
							Debug.WriteLine($"ClaReadPipeAsync PipeRead중 취소됨 : {oce.Message}");							
						}
						break;
					}
					catch (InvalidDataException ide)
					{
						...
						Debug.WriteLine($"ClaReadPipeAsync 데이터 파싱 중 오류 발생: {ide.Message}");
						// 필요에 따라 추가적인 작업을 수행할 수 있음
					}
					catch (Exception ex)
					{
						// 다른 예외 처리
						Debug.WriteLine($"ClaReadPipeAsync 예외 발생: {ex.Message}");
						// 예외 처리 로직 추가
					}
					finally
					{
			...
			{
				// 최상위 예외 처리
				Debug.WriteLine($"ClaReadPipeAsync 서버 읽기 중 예외 발생: {ex.Message}");
				// 예외 처리 로직 추가
			}
			finally
			{
