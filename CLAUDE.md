...
│  ├─ CONSISTENCY.md                # 일관성 모델 (무엇이 강하고 무엇이 최종 일관성인가)
...
### 8.1 공개 API는 승인 파일을 거친다

`Server/` 아래 모든 어셈블리에 `Microsoft.CodeAnalysis.PublicApiAnalyzers`가 걸려 있다.
public 표면을 바꾸면 **빌드가 실패한다.** 의도한 변경이면 승인 파일을 갱신한다.

| 파일 | 의미 |
|---|---|
| `PublicAPI.Shipped.txt` | 릴리스된 API. **여기서 줄을 지우면 파괴적 변경이다** |
| `PublicAPI.Unshipped.txt` | 아직 릴리스되지 않은 API. 릴리스 때 Shipped로 옮긴다 |

```bash
# 새 public 멤버를 추가한 뒤 승인 파일 갱신
dotnet format analyzers Server/<프로젝트>/<프로젝트>.csproj --diagnostics RS0016 --severity info
```

자동 적용이 안 되면 빌드 오류 메시지의 시그니처 문자열을 그대로
`PublicAPI.Unshipped.txt`에 넣는다. 형식이 정확해야 한다.

| 진단 | 뜻 | 할 일 |
|---|---|---|
| `RS0016` | 선언되지 않은 public 심볼 | 의도한 추가면 `Unshipped`에 넣는다 |
| `RS0017` | 승인 파일에 있는데 코드에 없음 | 의도한 제거·변경이면 그 줄을 지운다 |
| `RS0026` | 옵션 매개변수를 가진 오버로드 다중 정의 | **대개 설계를 고치는 게 맞다** (아래) |

**이 게이트의 목적은 diff 를 리뷰에 노출시키는 것이다.** 승인 파일의 변경이 곧
"제품 변경"이므로, 커밋에서 그 줄이 보이면 리뷰어가 의도를 물을 수 있다.
게이트를 끄거나 우회하지 않는다 — 나중에 켜면 이미 굳은 API를 되돌릴 수 없다.

> **RS0026을 만나면 기본값부터 의심한다.** 이 게이트를 켠 첫날 `FrameWriter.WriteFrameAsync`가
> 걸렸는데, 옵션 매개변수 세 개(`flags`, `sequence`, `cancellationToken`)의 기본값이 하필
> 레거시가 조용히 실패한 지점과 정확히 겹쳤다 — `cancellationToken = default`는 "종료 후에도
> 쓰기 시도", `sequence = 0`은 "있는 척하는 헤더 필드", `flags = None`은 "압축이 한 번도
> 실행되지 않음". 전부 필수로 바꾸는 것이 분석기 회피가 아니라 더 나은 계약이었다.

### 8.2 주석은 한글로, 모듈 단위로 상세하게

**모든 주석과 XML 문서는 한글로 쓴다.** 식별자·타입명은 영문이고, 설명은 한글이다.
이 프로젝트의 산출물은 프레임워크이고, **문서가 제품의 일부**다.

코드를 작성할 때 다음 4계층을 빠뜨리지 않는다. 위로 갈수록 "왜"의 비중이 크다.

| 계층 | 무엇을 적는가 | 분량 기준 |
|---|---|---|
| **어셈블리** | `docs/ARCHITECTURE.md`에 이 어셈블리가 어느 축인지, 무엇을 참조하고 무엇을 참조하지 않는지 | 문단 단위 |
| **파일/모듈** | 파일 최상단 타입의 `<summary>`+`<remarks>`. **이 타입이 왜 존재하는가**, 어떤 대안을 버렸는가, 어떤 규약을 지켜야 하는가 | 이 계층이 가장 상세하다 |
| **멤버** | public 멤버 전부 `<summary>`. 인자·반환·예외·수명 규약을 `<param>`/`<returns>`/`<exception>`/`<remarks>`로 | 한 줄이라도 반드시 |
| **본문** | 비자명한 한 줄에만. 비트 연산, 락-프리 순서, 매직 넘버, 성능 트릭 | 최소한으로 |

**모듈 주석에 반드시 담을 것**

1. **존재 이유** — 이 타입이 없으면 무엇이 깨지는가
2. **스레드 규약** — 스레드 안전한가, 아니면 어느 실행 컨텍스트 전용인가 (9.7)
3. **수명·소유권 규약** — 버퍼·풀 대여물을 누가 언제 반납하는가 (레거시 결함 원인 A)
4. **레거시 대응** — 승계 대상이라면 `docs/legacy/`의 어떤 결함을 막고 있는지 명시.
   "이 코드가 무엇을 하는가"보다 **"어떤 재발을 막는가"**가 더 오래 쓸모 있다
5. **성능 근거** — 무할당·SIMD·캐시 정렬 주장에는 수치나 링크 (2절 "측정 없는 최적화 금지")

**하지 말 것**

- 코드를 그대로 옮긴 주석 (`// i를 1 증가시킨다`)
- 영문 주석. 기존 한글 주석과 섞이면 읽는 비용만 는다
- 규약을 주석에만 적고 타입으로 강제하지 않기 — 주석은 보조이지 대체가 아니다 (9.7)
...
- **public 표면을 바꾸면 승인 파일도 갱신한다**(8.1). 빌드가 막히면 게이트를 끄지 말고 파일을 고친다.
- 코드를 쓰기 전에 **어느 축에 속하는지, 어느 어셈블리에 들어가는지** 먼저 확정한다. Core에 들어갈 후보라면 서드파티 의존이 없는지 먼저 확인한다.
- **주석은 한글로, 8.2의 4계층을 채운다.** 특히 모듈(파일 최상단 타입) 주석에 존재 이유·스레드 규약·수명 규약·막고 있는 레거시 결함을 적는다. 코드만 남기고 넘어가지 않는다.
