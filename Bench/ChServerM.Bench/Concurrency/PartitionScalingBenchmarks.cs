...
public class PartitionScalingBenchmarks
{
    ...
    /// <summary>파티션 수. 물리 코어 구간과 SMT 구간을 함께 덮는다.</summary>
    /// <remarks>
    /// <para>
    /// 2의 거듭제곱(1·2·4·8·16)에 ENV-A 의 물리 코어 수(12)와 논리 코어 수(24)를 더했다.
    /// <b>판정은 그 환경의 물리 코어 수까지만 본다</b>(ENV-A 는 12, ENV-B 는 16).
    /// 그 이상을 포함하는 이유는 <b>SMT·초과 구독 구간에서 무너지지 않는지</b>를 보기
    /// 위해서다 — 처리량이 오히려 떨어지면 경합 문제가 있다는 신호다.
    /// </para>
    /// <para>
    /// <b>⚠ 이 스윕만으로는 "코어 수 대비 확장"을 증명하지 못한다.</b> 파티션 수를 줄여도
    /// OS 는 여전히 모든 코어를 쓸 수 있다. 진짜 코어 제한은 프로세스 어피니티로 밖에서
    /// 감싸야 하며, <b>SMT 형제가 인접 쌍</b>이라 물리 코어 N 개의 마스크는 <b>한 칸씩
    /// 건너뛴</b> 비트다(2026-08-07 실측 확인 — 상세는 <c>Program.cs</c>).
    /// </para>
    /// </remarks>
    [Params(1, 2, 4, 8, 12, 16, 24)]
